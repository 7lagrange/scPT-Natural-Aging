...
gila <- subset(
  mouse,
  subset = seurat_clusters %in% c(14,16,17,21,35,37)
)
...
p1 <- DimPlot(gila, reduction = "umap", group.by="seurat_clusters",  label = T, label.size = 5) + ggtitle("brainHip InN re-cluster")
...
write.csv(meta, file = "data/brainHip_InN_metadata_with_double_manual.csv", row.names = FALSE)
