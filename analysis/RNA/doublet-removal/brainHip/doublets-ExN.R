...
DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, label=T,
         cells.highlight = WhichCells( mouse, idents = '28' ), cols = "grey85", cols.highlight = "red" 
) + ggtitle(paste0("all-cells-cluster 28 in ExN subcluster"))

DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, label=T,
         cells.highlight = WhichCells( mouse, idents = '41' ), cols = "grey85", cols.highlight = "red" 
) + ggtitle(paste0("all-cells-cluster 41 in ExN subcluster"))
...
gila$one_vs_others <- ifelse(
  ...
  ifelse(gila$seurat_clusters %in% c(8), "exclude", "others")
)
...
############# cluster 10 ##########  9 10 13 15 16 17 18 
...
############# cluster 13 ##########  9 10 13 15 16 17 18 
...
######## cluster 15 vs other CA3
gila$one_vs_others <- ifelse(
  gila$seurat_clusters == cluster_id, cluster_id,
  ifelse(gila$seurat_clusters %in% c(3,9), "others", "exclude")
)
Idents(gila) <- "one_vs_others"
markers <- FindAllMarkers(
  subset(gila, one_vs_others != "exclude"),
...
doublet_clusters <-c(13, 15)
gila$double_manual[gila$seurat_clusters %in% doublet_clusters] <- "doublet"
DimPlot(gila, reduction = "umap", group.by="double_manual",  label = T, label.size = 5) + ggtitle("ExN sub cluster doublets")


meta <- gila@meta.data
meta$cell_id <- rownames(meta)
write.csv(meta, file = "data/brainHip_ExN_metadata_with_double_manual.csv", row.names = FALSE)


saveRDS(gila, file = "data/mouse_ExN.rds")
gila<- readRDS("data/mouse_ExN.rds")
