...
gila <- subset(
  mouse,
  subset = seurat_clusters %in% c(1,39,33,38,32,27,23,30,42,25,26,40,13,36,22,6)
)
...
p1 <- DimPlot(gila, reduction = "umap", group.by="seurat_clusters",  label = T, label.size = 5) + ggtitle("brainHip Glia re-cluster")
p2 <- DimPlot(gila, reduction = "umap", group.by="scDblFinder.class",  label = T, label.size = 5) + ggtitle("brainHip scDblFinder.class")
...
gila$one_vs_others <- ifelse(
  ...
  ifelse(gila$seurat_clusters %in% c(4), "exclude", "others")
)
...
p2 <- FeaturePlot(mouse, features = "cluster_marker1", reduction = "umap", label = TRUE, order=T,
                  cols = c("grey90", "blue", "red", "darkred")) + ggtitle(paste0("Top 6 Markers Module Score for cluster", cluster_id, " in brainHip"))
p1 + p2
...
gila$one_vs_others <- ifelse(
  gila$seurat_clusters == cluster_id, cluster_id,
  ifelse(gila$seurat_clusters %in% c(9), "exclude", "others")
)
Idents(gila) <- "one_vs_others"
markers <- FindAllMarkers(
  subset(gila, one_vs_others != "exclude"),
...
DimPlot(gila, reduction = "umap", group.by="double_manual",  label = T, label.size = 5) + ggtitle("brainHip Glia doublets")
...
gila$one_vs_others <- ifelse(
  gila$seurat_clusters == cluster_id, cluster_id,
  ifelse(gila$seurat_clusters %in% c(18), "exclude", "others")
)
Idents(gila) <- "one_vs_others"
markers <- FindAllMarkers(
  subset(gila, one_vs_others != "exclude"),
...
gila$one_vs_others <- ifelse(
  gila$seurat_clusters == cluster_id, cluster_id,
  ifelse(gila$seurat_clusters %in% c(19), "exclude", "others")
)
Idents(gila) <- "one_vs_others"
markers <- FindAllMarkers(
  subset(gila, one_vs_others != "exclude"),
...
DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, label=T,
         cells.highlight = WhichCells( mouse, idents = '33' ), cols = "grey85", cols.highlight = "red" 
) + ggtitle(paste0("all-cells-cluster 33 in Glia subcluster"))

DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, label=T,
         cells.highlight = WhichCells( mouse, idents = '40' ), cols = "grey85", cols.highlight = "red" 
) + ggtitle(paste0("all-cells-cluster 40 in Glia subcluster"))

DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, label=T,
         cells.highlight = WhichCells( mouse, idents = '42' ), cols = "grey85", cols.highlight = "red" 
) + ggtitle(paste0("all-cells-cluster 42 in Glia subcluster"))

DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, label=T,
         cells.highlight = WhichCells( mouse, idents = '22' ), cols = "grey85", cols.highlight = "red" 
) + ggtitle(paste0("all-cells-cluster 22 in Glia subcluster"))

DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, label=T,
         cells.highlight = WhichCells( mouse, idents = '26' ), cols = "grey85", cols.highlight = "red" 
) + ggtitle(paste0("all-cells-cluster 26 in Glia subcluster"))




gila$double_manual <- "singlet"
doublet_clusters <-c(4,9,14,18,19)
gila$double_manual[gila$seurat_clusters %in% doublet_clusters] <- "doublet"
...
write.csv(meta, file = "data/brainHip_Glia_metadata_with_double_manual.csv", row.names = FALSE)
