...
meta <- mouse@meta.data[mouse$seurat_clusters %in% c(31,34), ]
...
meta <- gila@meta.data[gila$seurat_clusters %in% c(13,15), ]
meta$barcode <- sub("^[^:]+:", "", meta$full_barcode_with_batch)
meta$cell_id <- paste0(meta$SimpleID, ":", meta$barcode)
...
DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", label = TRUE, label.size = 4 )
meta <- gila@meta.data[gila$seurat_clusters %in% c(8,11,12,14), ]
meta$barcode <- sub("^[^:]+:", "", meta$full_barcode_with_batch)
meta$cell_id <- paste0(meta$SimpleID, ":", meta$barcode)
...
meta <- gila@meta.data[gila$seurat_clusters %in% c(4,9,14,18,19), ]
meta$barcode <- sub("^[^:]+:", "", meta$full_barcode_with_batch)
meta$cell_id <- paste0(meta$SimpleID, ":", meta$barcode)
...
# 合并
final_meta <- bind_rows(all_meta, all_meta_pred, exn_meta, ihn_meta, glia_meta)
final_meta <- distinct(final_meta, cell_id, .keep_all = TRUE)
write.csv(final_meta, file = "brainHip_doublet_cells_combined.csv", row.names = FALSE)
