...
saveRDS(gila, file = "data/mouse_ExN_high_res.rds")
#########
...
cells_9 <- WhichCells(gila, idents = "9")
cells_9_mouse <- intersect(cells_9, colnames(mouse))
table(mouse$seurat_clusters[cells_9_mouse])
...
gila$one_vs_others <- ifelse(
  gila$seurat_clusters == cluster_id, cluster_id,
  ifelse(gila$seurat_clusters %in% c(20), "exclude", "others")
)
Idents(gila) <- "one_vs_others"
markers <- FindAllMarkers(
  subset(gila, one_vs_others != "exclude"),
...
DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, 
               cells.highlight = WhichCells( mouse, idents = '39' ), cols = "grey85", cols.highlight = "red" 
         ) + ggtitle(paste0("all-cells-cluster 39 in ExN subcluster"))

DimPlot( gila, reduction = "umap", group.by = "seurat_clusters", raster=FALSE, 
         cells.highlight = WhichCells( mouse, idents = '11' ), cols = "grey85", cols.highlight = "red" 
         ) + ggtitle(paste0("all-cells-cluster 11 in ExN subcluster"))
...
df_umap <- data.frame(
  ...
  cluster = Idents(mouse)
)
cells_11 <- WhichCells(mouse, idents = "11")
cells_11_low  <- df_umap$cell[df_umap$cell %in% cells_11 & df_umap$UMAP_2 <  -6]
cells_11_high <- df_umap$cell[df_umap$cell %in% cells_11 & df_umap$UMAP_2 >= -6]
...
p1 <- DimPlot(
  ...
  raster = FALSE,
  cells.highlight = cells_11_low,
  cols = "grey85",
  cols.highlight = "blue"
) + ggtitle("Cluster 11: UMAP_2 < -6")
...
doublet_clusters <-c(9)
gila$double_manual[gila$seurat_clusters %in% doublet_clusters] <- "doublet"
DimPlot(gila, reduction = "umap", group.by="double_manual",  label = T, label.size = 5) + ggtitle("brainFC ExN sub cluster all doublets")
