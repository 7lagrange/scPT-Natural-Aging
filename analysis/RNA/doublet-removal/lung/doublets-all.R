...
setwd("~/github/zhanglab-code/projects/paired_seq_tag/code/machine-experiments/RNA/doublet/lung")
...
rds_path <- "~/github/zhanglab-code/projects/paired_seq_tag/code/machine-experiments/RNA/cell-anno/lung/data/lung_cell_anno.rds"
...
p1 <- DimPlot(mouse, reduction = "umap_harmony", group.by="seurat_clusters",  label = T, label.size = 5) + ggtitle("lung cluster")
DimPlot(mouse, reduction = "umap_harmony", group.by="scDblFinder.class",  label = T, label.size = 5) + ggtitle("lung scDblFinder.class")
...
FeaturePlot(mouse, features = c('Sftpc', 'Pecam1', 'Pdgfra', 'leiden'), ncol = 3, reduction = "umap_harmony", order=T, label=T) 
DotPlot(mouse, features = c('Sftpc', 'Pecam1', 'Pdgfra', 'leiden'), group.by = "seurat_clusters", scale = T) 
...
mouse$doublet <- ifelse(
  mouse$scDblFinder.class == "doublet" |
    mouse$seurat_clusters %in% c(22, 24,28,31,32,33,34,35,36,37),
  "doublet",
  "singlet"
)
...
DimPlot(subset(mouse, subset = cell_type != "proliferating_cells"), reduction = "umap_harmony", group.by="cell_type",  label = T, label.size = 5)

table(mouse$cell_type, mouse$seurat_clusters)


mouse <- subset(mouse, subset = cell_type != "proliferating_cells")
...
mouse$doublet <- ifelse(mouse$seurat_clusters %in% c(21,22,23,24,25,26),
  "doublet",
  "singlet"
)
...
mouse <- subset(mouse, subset = doublet == "singlet")
DimPlot( mouse, reduction = "umap_harmony", group.by = "cell_type", label = TRUE, label.size = 4 )
...
saveRDS(mouse, file = "data/lung_cell_remode_doublets.rds")

mouse <- readRDS("data/lung_cell_remode_doublets.rds")
